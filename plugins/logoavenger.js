let handler = async(m, { conn, text }) => {
    ...
    let link = 'https://arugaz.my.id/api/textpro/avengers?text1=' + l + '&text2=' + r

    conn.sendFile(m.chat, link, 'Nih bro')
}
handler.help = ['logoavengers', 'logoavenger'].map(v => v + ' <teks>|<teks>')
handler.tags = ['tools']
handler.command = /^(logoavengers|logoavenger)$/i
handler.owner = false
handler.mods = false
